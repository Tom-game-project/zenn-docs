...
その代わりに、ジャンプラベル、Hold/Patternスペースのような特徴的な概念があります。sedで計算らしいことをするためにはまずこれらの概念をインストールする必要があるでしょう。
...
まず、加算器を実装してみました。init commitを見ればadd.sedが作られていることがわかるでしょう。
なんとなく扱いやすいかなーという理由で数値のデータは二進数で管理しています。
...
残る演算、除算及び余りの計算のため、試行錯誤しつつ、pythonなどの実装を参考にしてIRで実装しようと考えていました。
がしかし、除算は若干複雑で、IRに変換する作業はかなり苦痛でした。そのため、IRよりもさらに高級な言語を作り解決します。
sedが埋め込める必要があるので、それを意識しつつ実装していきます。
