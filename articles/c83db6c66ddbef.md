...
今年触って最も感動したソフトウェアといっても過言ではないです。chumskyはそれ自体がRustの型システムをふんだんに使い作られていて、ある程度使い方側わかればとても簡単に、そして安全にパーサーを構築することができます。今度別個記事にまとめられたらいいと考えています。
...
スタックフレーム上で引数がどのように扱われるか理解する必要があり少し大変ではある
...
私以外の人も使えるようなインターフェイスを作らねばと思っています。cliがエントリーポイントとなり、処理が追いやすくなると思います。

### 色々なデータ操作

今回は数値計算と関数の実現に特化した内容でしたが、sedの本来の威力は文字列操作です。文字列を簡単に操作できるようにsoil側でデータを受け付けられるようにしたいです。
